import type { NewsCategory, WorkStatus } from "@/types/content";

export const workStatusLabel: Record<WorkStatus, string> = {
  active: "En cartel",
  archive: "Archivo",
};

export const workStatusStyles: Record<WorkStatus, string> = {
  active: "border border-orange-300/20 bg-orange-400/10 text-orange-100",
  archive: "border border-white/12 bg-white/8 text-white",
};

export const newsCategoryLabel: Record<NewsCategory, string> = {
  estreno: "Estreno",
  festival: "Festival",
  taller: "Taller",
  prensa: "Prensa",
  anuncio: "Anuncio",
};

export const newsCategoryStyles: Record<
  NewsCategory,
  {
    chip: string;
    surface: string;
  }
> = {
  estreno: {
    chip: "border border-orange-300/20 bg-orange-400/10 text-orange-100",
    surface:
      "border-orange-400/15 bg-[linear-gradient(145deg,rgba(244,92,44,0.16),rgba(18,18,18,0.96))]",
  },
  festival: {
    chip: "border border-rose-300/20 bg-rose-400/10 text-rose-100",
    surface:
      "border-rose-400/15 bg-[linear-gradient(145deg,rgba(138,30,44,0.24),rgba(16,16,16,0.96))]",
  },
  taller: {
    chip: "border border-amber-300/20 bg-amber-300/10 text-amber-100",
    surface:
      "border-amber-300/15 bg-[linear-gradient(145deg,rgba(255,176,89,0.16),rgba(16,16,16,0.95))]",
  },
  prensa: {
    chip: "border border-white/15 bg-white/8 text-white",
    surface:
      "border-white/10 bg-[linear-gradient(145deg,rgba(255,255,255,0.05),rgba(16,16,16,0.96))]",
  },
  anuncio: {
    chip: "border border-orange-300/18 bg-orange-300/10 text-orange-50",
    surface:
      "border-orange-300/14 bg-[linear-gradient(145deg,rgba(161,28,33,0.2),rgba(17,17,17,0.96))]",
  },
};
